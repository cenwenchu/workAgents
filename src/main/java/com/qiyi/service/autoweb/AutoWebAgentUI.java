...
import com.qiyi.service.autoweb.AutoWebAgent.ContextWrapper;
import com.qiyi.service.autoweb.AutoWebAgent.HtmlSnapshot;
import com.qiyi.service.autoweb.AutoWebAgent.HtmlCaptureMode;
import com.qiyi.service.autoweb.AutoWebAgent.ModelSession;
import com.qiyi.service.autoweb.AutoWebAgent.PlanParseResult;
import com.qiyi.service.autoweb.AutoWebAgent.PlanStep;
...
import static com.qiyi.service.autoweb.AutoWebAgent.*;
...
class AutoWebAgentUI {
        ...
        btnStepExecute.addActionListener(e -> {
            ...
            Thread execThread = new Thread(() -> {
                try {
                    ...
                    try { sharedBinding.setVariable("allGoodsData", new java.util.ArrayList<>()); } catch (Exception ignored) {}
    ...
    private static Object evalInContext(Object ctx, String script) {
        if (ctx instanceof com.microsoft.playwright.Frame f) return f.evaluate(script);
        if (ctx instanceof Page p) return p.evaluate(script);
        return null;
    }

    private static Object evalInContext(Object ctx, String script, Object arg) {
        if (ctx instanceof com.microsoft.playwright.Frame f) return f.evaluate(script, arg);
        if (ctx instanceof Page p) return p.evaluate(script, arg);
        return null;
    }

    private static java.util.Map<String, Object> readBestScrollerState(Object ctx) {
        if (ctx == null) return java.util.Collections.emptyMap();
        try {
            Object stateObj = evalInContext(ctx, "() => {\n" +
                    "  const el = window.__autowebBestScroller;\n" +
                    "  if (el && el !== window) {\n" +
                    "    return { mode: 'el', tag: (el.tagName || ''), y: (el.scrollTop || 0), ih: (el.clientHeight || 0), sh: (el.scrollHeight || 0) };\n" +
                    "  }\n" +
    ...
    private static com.microsoft.playwright.Frame pickLargestVisibleChildFrame(Page page) {
        if (page == null) return null;
        com.microsoft.playwright.Frame best = null;
        double maxArea = 0;
        try {
            for (com.microsoft.playwright.Frame f : page.frames()) {
                if (f == null) continue;
                try { if (f == page.mainFrame()) continue; } catch (Exception ignored) {}
    ...
    private static double[] computeWheelPoint(Page page, Object scrollCtx) {
        if (page == null) return new double[]{0, 0};
        if (scrollCtx instanceof com.microsoft.playwright.Frame f) {
            try {
                ElementHandle el = f.frameElement();
                if (el != null) {
                    com.microsoft.playwright.options.BoundingBox box = el.boundingBox();
                    if (box != null && box.width > 0 && box.height > 0) {
                        ...
                        double y = box.y + Math.max(1, Math.min(box.height - 1, box.height / 2.0));
        ...
        try {
            Object ptObj = page.evaluate("() => {\n" +
                    ...
                    "  const y = (window.innerHeight || 0) * 0.6;\n" +
                    "  return { x, y };\n" +
                    "}");
            if (ptObj instanceof java.util.Map<?, ?> mpt) {
                double mx = asDouble(mpt.get("x"), 0);
                double my = asDouble(mpt.get("y"), 0);
    ...
    private static java.util.List<java.io.File> captureMultiScreenScreenshots(Page page, java.util.function.Consumer<String> uiLogger) {
        ...
        synchronized (PLAYWRIGHT_LOCK) {
            try {
                ...
                Object scrollCtx = page;
                try {
                    com.microsoft.playwright.Frame bestFrame = pickLargestVisibleChildFrame(page);
                    if (bestFrame != null) {
                        scrollCtx = bestFrame;
                        traceVisual("VISUAL_SCROLL_CTX", "frame=" + bestFrame.name() + ", url=" + bestFrame.url());
                        if (uiLogger != null) uiLogger.accept("视觉补充: 采用主内容 Frame 滚动（" + bestFrame.name() + "）");
                    } else {
                        traceVisual("VISUAL_SCROLL_CTX", "page");
                    }
                } catch (Exception ignored) {
                    scrollCtx = page;
                    traceVisual("VISUAL_SCROLL_CTX", "page");
                ...
                Object initObj = null;
                try {
                    initObj = evalInContext(scrollCtx, "() => {\n" +
                            "  const winH = window.innerHeight || 0;\n" +
                            "  const winW = window.innerWidth || 0;\n" +
                            "  const cx = winW / 2;\n" +
                            "  const cy = Math.max(1, Math.min(winH - 1, winH * 0.6));\n" +
                            "  const overflowOk = (oy) => (oy === 'auto' || oy === 'scroll' || oy === 'overlay');\n" +
                            "  const visibleRect = (r) => !!r && (r.width || 0) > 0 && (r.height || 0) > 0 && (r.bottom || 0) > 0 && (r.right || 0) > 0 && (r.left || 0) < winW && (r.top || 0) < winH;\n" +
                            "  const isScrollable = (el) => {\n" +
                            "    if (!el) return false;\n" +
                            "    let st;\n" +
                            "    try { st = window.getComputedStyle(el); } catch (e) { st = null; }\n" +
                            "    const oy = st ? (st.overflowY || '') : '';\n" +
                            "    if (!overflowOk(oy)) return false;\n" +
                            "    const ch = el.clientHeight || 0;\n" +
                            "    const sh = el.scrollHeight || 0;\n" +
                            "    const cw = el.clientWidth || 0;\n" +
                            "    if (sh <= ch + 40) return false;\n" +
                            "    if (ch < Math.max(200, Math.floor(winH * 0.45))) return false;\n" +
                            "    if (cw < Math.max(260, Math.floor(winW * 0.35))) return false;\n" +
                            "    let r;\n" +
                            ...
                            "    return null;\n" +
                            "  };\n" +
                            "  let best = pickFromCenter();\n" +
                            "  let bestScore = 0;\n" +
                            "  let strategy = best ? 'center' : 'scan';\n" +
                            "  if (!best) {\n" +
                            ...
                            "      const inside = cx >= (r.left || 0) && cx <= ((r.left || 0) + (r.width || 0)) && cy >= (r.top || 0) && cy <= ((r.top || 0) + (r.height || 0));\n" +
                            "      let dx = 0;\n" +
                            "      let dy = 0;\n" +
                            "      if (!inside) {\n" +
                            "        const rx0 = (r.left || 0);\n" +
                            "        const rx1 = rx0 + (r.width || 0);\n" +
                            "        const ry0 = (r.top || 0);\n" +
                            "        const ry1 = ry0 + (r.height || 0);\n" +
                            "        if (cx < rx0) dx = rx0 - cx;\n" +
                            "        else if (cx > rx1) dx = cx - rx1;\n" +
                            "        if (cy < ry0) dy = ry0 - cy;\n" +
                            "        else if (cy > ry1) dy = cy - ry1;\n" +
                            "      }\n" +
                            "      const dist = Math.sqrt(dx * dx + dy * dy);\n" +
                            "      const centerWeight = inside ? 6 : (1 / (1 + dist / 120));\n" +
                            "      const score = (sh - ch) * cw * centerWeight;\n" +
                            "      if (score > bestScore) {\n" +
                            "        bestScore = score;\n" +
                            "        best = el;\n" +
                            "      }\n" +
                            ...
                            "  return { ok: true, strategy, hasEl: !!best, tag: best ? (best.tagName || '') : '', score: bestScore };\n" +
                            "}");
                } catch (Exception ignored) {
                    initObj = null;
                }
                ...
                double originY = 0;
                try {
                    java.util.Map<String, Object> s0 = readBestScrollerState(scrollCtx);
                ...
                for (int i = 1; i <= maxScreens; i++) {
                    ...
                    String tag = "";
                    try {
                        java.util.Map<String, Object> s1 = readBestScrollerState(scrollCtx);
                    ...
                    try {
                        ...
                        java.util.Map<String, Object> s2 = readBestScrollerState(scrollCtx);
                        ...
                        if (Math.abs(afterY - y) < 2) {
                            try {
                                ...
                                double[] pt = computeWheelPoint(page, scrollCtx);
                                double mx = pt[0];
                                double my = pt[1];
                                ...
                                page.waitForTimeout(350);
                                java.util.Map<String, Object> s3 = readBestScrollerState(scrollCtx);
                ...
                try {
                    ...
                    java.util.Map<String, Object> sr = readBestScrollerState(scrollCtx);
