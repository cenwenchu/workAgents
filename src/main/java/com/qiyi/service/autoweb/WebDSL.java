...
public class WebDSL {
    ...
    private Frame ensureFrame() {
        ...
        Frame recovered = null;
        if (savedFrameName != null && !savedFrameName.isEmpty()) {
            recovered = findFrameByName(savedFrameName);
            if (recovered != null) {
                this.frame = recovered;
        ...
        if (savedFrameUrl != null && !savedFrameUrl.isEmpty()) {
            recovered = findFrameByUrl(savedFrameUrl);
            if (recovered != null) {
                this.frame = recovered;
    ...
    private Frame findFrameByName(String name) {
        ...
        try {
            for (Frame f : page.frames()) {
                if (f == null) continue;
                try { if (f.isDetached()) continue; } catch (Exception ignored) {}
                try {
    ...
    private Frame findFrameByUrl(String url) {
    ...
    private void switchToFrame(Frame f) {
        this.frame = f;
        if (f == null) {
            this.savedFrameName = null;
            this.savedFrameUrl = null;
            return;
        }
        try {
            this.savedFrameName = f.name();
            this.savedFrameUrl = f.url();
        } catch (Exception ignored) {}
    }
    ...
    private static final class ContextCandidate {
        ...
        ContextCandidate(Frame frame, double area, boolean visible) {
            this.frame = frame;
            this.area = area;
    ...
    private double viewportArea() {
        try {
            ...
            if (vs != null) return Math.max(0, (double) vs.width * (double) vs.height);
        } catch (Exception ignored) {}
        try {
            Object v = page.evaluate("() => ({ w: window.innerWidth || 0, h: window.innerHeight || 0 })");
            if (v instanceof java.util.Map) {
                Object w = ((java.util.Map<?, ?>) v).get("w");
                Object h = ((java.util.Map<?, ?>) v).get("h");
                double ww = (w instanceof Number) ? ((Number) w).doubleValue() : 0;
                double hh = (h instanceof Number) ? ((Number) h).doubleValue() : 0;
                return Math.max(0, ww * hh);
            }
        } catch (Exception ignored) {}
        return 0;
    }

    private ContextCandidate toCandidate(Frame f) {
        if (f == null) return new ContextCandidate(null, viewportArea(), true);
        try {
            if (f == page.mainFrame()) return null;
        } catch (Exception ignored) {}
        try {
            if (f.isDetached()) return null;
        } catch (Exception ignored) {}
        try {
            ElementHandle el = f.frameElement();
            if (el == null) return new ContextCandidate(f, 0, false);
            com.microsoft.playwright.options.BoundingBox box = el.boundingBox();
            if (box == null) return new ContextCandidate(f, 0, false);
            double w = Math.max(0, box.width);
            double h = Math.max(0, box.height);
            double area = w * h;
            boolean visible = w > 0 && h > 0;
            return new ContextCandidate(f, area, visible);
        } catch (Exception ignored) {}
        return new ContextCandidate(f, 0, false);
    }

    private java.util.ArrayList<ContextCandidate> listContextCandidatesByVisibleArea() {
        java.util.ArrayList<ContextCandidate> out = new java.util.ArrayList<>();
        out.add(new ContextCandidate(null, viewportArea(), true));
        for (Frame f : page.frames()) {
            ContextCandidate c = toCandidate(f);
            if (c != null) out.add(c);
        }
        out.sort((a, b) -> {
    ...
    private java.util.ArrayList<ContextCandidate> listContextCandidatesForSearch() {
        Frame preferred = null;
        try { preferred = ensureFrame(); } catch (Exception ignored) {}
        java.util.ArrayList<ContextCandidate> sorted = listContextCandidatesByVisibleArea();
        java.util.ArrayList<ContextCandidate> out = new java.util.ArrayList<>();
        ContextCandidate first = toCandidate(preferred);
        if (first != null) out.add(first);
        for (ContextCandidate c : sorted) {
            if (preferred == null) {
                if (c.frame == null) continue;
            } else {
                if (c.frame == preferred) continue;
            }
            out.add(c);
        }
        return out;
    ...
    private Locator tryWaitForAcrossContexts(String selector, double totalTimeoutMs) {
        java.util.ArrayList<ContextCandidate> order = listContextCandidatesForSearch();
        double budget = Math.max(0, totalTimeoutMs);
        if (budget <= 0) budget = 1200;
        for (int i = 0; i < order.size(); i++) {
            double remain = Math.max(0, budget);
            int remainingSlots = Math.max(1, order.size() - i);
            double per = Math.max(250, Math.min(1500, remain / remainingSlots));
            Frame f = order.get(i).frame;
            try {
                Locator loc = locatorInContext(f, selector).first();
                loc.waitFor(new Locator.WaitForOptions().setTimeout(per));
                if (f != null) {
                    log("  -> Context fallback: switch to frame " + f.name() + " (" + f.url() + ")");
                } else {
                    log("  -> Context fallback: switch to main page");
                }
                switchToFrame(f);
                return loc;
            } catch (Exception ignored) {
    ...
    private Locator promoteToClickable(Locator loc) {
        if (loc == null) return null;
        try {
            Locator clickable = loc.locator("xpath=ancestor-or-self::*[self::button or self::a or self::label or self::option or self::input or self::select or self::textarea or @role='button' or @role='link' or @role='menuitem' or @role='tab' or @role='option' or @role='checkbox' or @role='radio' or @role='combobox'][1]").first();
            waitForLocatorAttached(clickable, 100);
            if (clickable.count() > 0) return clickable;
        } catch (Exception ignored) {}
        return loc;
    }
    ...
    public void click(String selector) {
        ...
        retry(() -> {
            ...
            try {
                if (loc.count() == 0) {
                    Locator cross = tryWaitForAcrossContexts(selector, Math.min(2000, defaultTimeout));
                    if (cross != null) loc = cross.first();
                }
            } catch (Exception ignored) {}

            try { loc = promoteToClickable(loc).first(); } catch (Exception ignored) {}
    ...
    public void waitFor(String selector) {
        ...
        try {
            ...
            Locator cross = tryWaitForAcrossContexts(selector, Math.min(2500, defaultTimeout));
    ...
    public void waitFor(String selector, Number timeout) {
        ...
        try {
            ...
            Locator cross = tryWaitForAcrossContexts(selector, Math.min(2500, timeout.doubleValue()));
            if (cross != null) return;
            RuntimeException login = rewriteIfLogin(e, "waitFor " + selector);
            if (login != null) throw login;
            throw e;
    ...
    public void clickButton(String textOrSelector) {
        ...
        if (looksLikeTextRegex(raw)) {
            ...
            attempts = new String[] {
                    ...
                    "role=menuitem[name=/" + pattern + "/]",
                    "role=link[name=/" + pattern + "/]",
                    "a:has(p:has-text(/" + pattern + "/))",
                    "a[class*=\"menuItem\"]:has-text(/" + pattern + "/)",
                    "text=/" + pattern + "/"
            };
        } else {
            String escaped = escapeForSelectorValue(raw);
        ...
        for (String sel : attempts) {
            ...
            try {
                Locator cross = tryWaitForAcrossContexts(sel, Math.min(2500, defaultTimeout));
                if (cross != null) {
                    click(sel);
                    return;
                }
