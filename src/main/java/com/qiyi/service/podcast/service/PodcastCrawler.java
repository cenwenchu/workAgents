...
public class PodcastCrawler {
    ...
    public List<PodCastItem> scanForNewEpisodes(int maxProcessCount, int maxTryTimes, int maxDuplicatePages, List<String> existingItemNames) {
        ...
        try {
            ...
            AppLog.error("扫描任务出错: " + e.getMessage());
            AppLog.error(e);
        } finally {
    ...
    private boolean navigateToFollowing(Page page) {
        try {
            ...
            AppLog.error("导航到 Following 失败: " + e.getMessage());
        }
        return false;
    }
    ...
    private void filterReadyPodcasts(Page page) {
        try {
            ...
            AppLog.error("筛选 Ready 状态失败: " + e.getMessage());
        }
    }
        ...
        do {
            ...
            if (elements.size() > lastProcessedIndex) {
                    ...
                    try {
                        ...
                        if (item != null && !existingNames.contains(item.title)) {
                            if (item.isProcessed) {
                                ...
                                AppLog.info("找到有效Item: " + item.channelName + " - " + item.title);
                                showPageNotification(page, "发现新可下载播客: " + item.title);
                            } else {
                                // 调试日志：记录未就绪的项目
                                 AppLog.info("Item 未就绪 (Skipped): " + item.title);
                            }
                        }
                        else
                        {
                            if (item != null && existingNames.contains(item.title)) {
                                AppLog.info("Item 重复 (Skipped): " + item.title);
                            }
                            else
                            {
                                AppLog.info("Item 不存在 (Skipped)");
                            }
                        }
                    } catch (Exception e) {
                        AppLog.error("处理单个元素时发生异常 (Index: " + i + "), 继续处理下一个: " + e.getMessage());
                    }
                }
                ...
                if (!hasNewValidItemInThisBatch) {
                    ...
                    AppLog.info("连续 " + consecutiveDuplicatePages + " 次下拉未发现新数据");

                    if (consecutiveDuplicatePages >= maxDuplicatePages) {
                        AppLog.info("连续 " + maxDuplicatePages + " 次下拉未发现新数据，提前结束");
                        break;
                    }
                } else {
    ...
    private PodCastItem parsePodcastItem(ElementHandle element) {
        ...
        try {
            ...
            {
                AppLog.info("Item 被跳过: " + item.toString());
                return null;
            }
        } catch (Exception e) {
            AppLog.error("解析Item失败: " + e.getMessage());
            return null;
        }
    }
    ...
    private boolean scrollToLoadMore(Page page) {
        try {
            ...
            AppLog.info("已滚动到底部，等待加载...");
            PodCastUtil.waitForHeightStabilized(page, 10);
            return true;
        } catch (Exception e) {
            AppLog.error("滚动失败: " + e.getMessage());
            return false;
        }
    }
    ...
    public void downloadEpisode(PodCastItem item, String savePath, String cnSavePath) {
        ...
        try {
                ...
                if (pdfButton != null) {
                    ...
                    if (downloadBtn != null) {
                        ...
                        page.onDownload(download -> {
                            AppLog.info("下载开始: " + download.url());
                            AppLog.info("建议的文件名: " + download.suggestedFilename());
                            try {
                        ...
                        if (tempPath != null && tempPath.toFile().exists()) {
                            ...
                            if (Files.exists(defaultDownloadPath)) {
                                AppLog.info("从默认下载目录找到文件: " + defaultDownloadPath);
                                ...
                                AppLog.info("默认下载目录未找到文件，尝试 saveAs");
                        ...
                        AppLog.info("English download: " + savePath);
                        try {
            ...
            AppLog.error("Download failed [" + item.title + "]: " + e.getMessage());
                ...
                if (cnBtn != null) {
                    ...
                    if (newDownloadBtn != null) {
                        ...
                        if (tempPath != null && tempPath.toFile().exists()) {
                             ...
                             if (Files.exists(defaultDownloadPath)) {
                                 AppLog.info("从默认下载目录找到文件(CN): " + defaultDownloadPath);
                                 java.nio.file.Files.copy(defaultDownloadPath, targetPath, java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                                 // 可选：删除默认下载目录中的文件
                                 Files.delete(defaultDownloadPath); 
                             } else {
                        ...
                        AppLog.info("Chinese download: " + savePath);
                        try { showPageNotification(page, "中文版本下载完成: " + savePath); } catch (Exception ignore) {}
                    }
                }
            }
        } catch (Exception e) {
            AppLog.error("CN download failed: " + e.getMessage());
