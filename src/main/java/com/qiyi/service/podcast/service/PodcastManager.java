package com.qiyi.service.podcast.service;
...
public class PodcastManager {
    ...
    public int runDownloadTask(int maxProcessCount, int maxTryTimes, int maxDuplicatePages, boolean onlyReadReady, ModelType modelType, int maxRenameBatchSize) {
        AppLog.info("Starting Download Task...");
        ...
        if (!fileService.fileListExists()) {
            ...
            AppLog.info("File list exists, skipping scan.");
            itemsToDownload = fileService.readItemListFromFile();
            // 如果读取的文件列表数量超过了本次要求的最大数量，进行截断
            if (itemsToDownload.size() > maxProcessCount) {
                AppLog.info("Pending list size (" + itemsToDownload.size() + ") exceeds maxProcessCount (" + maxProcessCount + "), truncating...");
        ...
        for (PodCastItem item : itemsToDownload) {
             ...
             if (new File(downloadPath).exists()) {
                 AppLog.info("File exists, skipping: " + downloadPath);
                 continue;
             }
    ...
    public void runRenameTask(ModelType modelType, int maxBatchSize) {
        ...
        if (files == null || files.length == 0) {
            AppLog.info("No CN files to rename.");
            return;
        }
        
        AppLog.info("Starting Rename Task for " + files.length + " files.");
    ...
    public void runProcessingTask(int maxProcessCount, ModelType modelType, boolean needGenerateImage, boolean isStreaming, int threadPoolSize, ToolContext context) {
        runProcessingTask(maxProcessCount, modelType, needGenerateImage, isStreaming, threadPoolSize, context, null);
    }

    public void runProcessingTask(int maxProcessCount, ModelType modelType, boolean needGenerateImage, boolean isStreaming, int threadPoolSize, ToolContext context, ToolMessenger messenger) {
        AppLog.info("Starting Processing (Summary/Image) Task...");
        ...
        if (targetFiles == null || targetFiles.length == 0) {
            AppLog.info("No PDF files found in " + dir.getPath());
            return;
        }

        try {
            String startMsg = "开始分析生成摘要，待处理文件数: " + targetFiles.length;
            if (messenger != null) {
                messenger.sendText(startMsg);
            } else {
                DING_TALK_SERVICE.sendTextMessageToEmployees(DING_TALK_SERVICE.getConfig().getPodcastAdminUsers(), startMsg);
            }
        } catch (Exception e) {
            AppLog.error(e);
        }
        ...
        for (File file : targetFiles) {
            ...
            if (summaryFile.exists()) {
                AppLog.info("Summary exists, skipping: " + summaryFileName);
                continue;
            }
        ...
        for (Future<?> f : futures) {
            try { f.get(); } catch (Exception e) { AppLog.error(e); }
        }

        PFileUtil.batchRenameChineseFiles(fileService.getDownloadDirSummary(),modelType, 50);
        
        executor.shutdown();
        AppLog.info("Processing Task Completed.");
        try {
            String doneMsg = "播客摘要分析生成完成，处理文件数: " + processedCount;
            if (messenger != null) {
                messenger.sendText(doneMsg);
            } else {
                DING_TALK_SERVICE.sendTextMessageToEmployees(DING_TALK_SERVICE.getConfig().getPodcastAdminUsers(), doneMsg);
            }
        } catch (Exception e) {
            AppLog.error(e);
        }
